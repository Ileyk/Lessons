...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Constant coefficients linear hyperbolic PDE}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\bul Homogeneous
\begin{equation}
\partial_t \mathbf{U} + \mathbf{A}\left(\mathbf{U},x,t\right) \partial_x \mathbf{U} = \mathbf{0}
\label{eq:QL_form}
\end{equation}

XXX Choice diagram XXX

\bul Non-homogeneous : $...+\mathbf{B}\left(\mathbf{U},x,t\right)=\mathbf{0}$
\iar still linear if $\mathbf{B}\propto\mathbf{U}$

Time and space dependent linear advection equation? % L w/ var coeff
Burgers equation? % QL
Conservation equations? % QL non homogeneous if source terms
...
\iar Matrix form $\partial_t \mathbf{U}+\XXX{A} \partial_x \mathbf{U}$?
\iar $\XXX{A}$ diagonalisable? 
\iar Diagonal matrix $\XXX{D}$?
\iar Eigenvector matrix $\XXX{K}$ such as $\XXX{A}=\XXX{K}\XXX{D}\XXX{K}^{—1}$?
...
\bul Elliptic <=> complex eigenvalues
...
\bul Hyperbolic <=> real eigenvalues and linearly independent eigenvectors
...
\bul If \textit{linear constant coeff.}, the procedure is the following

$\XXX{K}^{-1}\times$\eqref{eq:QL_form} => canonical form :
$$ \partial_t \mathbf{V} + \XXX{D} \partial_x \mathbf{V} = 0 $$
...
Given initial conditions $\mathbf{U} \left( x, 0)$, $\mathbf{V}$ basis of decoupled linear advection solutions :
$$ V_i \left( x, t \right) = V_i \left( x-\lambda_i t\right, 0) $$
...
\iar Exo: section 6.3 of Leveque
...
\end{equation}
with $\mathbf{AXXX} \left( \mathbf{U} \right)$ the flux Jacobian matrix :
